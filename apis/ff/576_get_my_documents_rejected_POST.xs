...
query getMyDocumentsRejected verb=POST {
  api_group = "ff"
  auth = "user"
    ...
    conditional {
      if ($input.documentClassification == 0) {
        db.query myDocuments {
          where = $db.myDocuments.company == $input.company && $db.myDocuments.active == true && $db.myDocuments.archived == false && $db.myDocuments.rejected == true
          sort = {
        ...
        conditional {
          if (($myPersonaShare1|is_empty) == false) {
            ...
            db.query myDocuments {
              where = $db.myDocuments.id in $myPersonaShare1 && $db.myDocuments.active == true && $db.myDocuments.archived == false && $db.myDocuments.rejected == true
              sort = {
                myDocuments.expiryDate: "asc"
                myDocuments.created_at: "desc"
              }
        ...
        db.query myDocuments {
          where = $db.myDocuments.company == $input.company && $db.myDocuments.active == true && $db.myDocuments.document in $documents1 && $db.myDocuments.archived == false && $db.myDocuments.rejected == true
          sort = {
            myDocuments.expiryDate: "asc"
            myDocuments.created_at: "desc"
          }
