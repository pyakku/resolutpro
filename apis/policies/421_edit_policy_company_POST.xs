query edit_policy_company verb=POST {
  api_group = "policies"
  ...
  input {
    dblink {
      ...
      override = {
        ...
        global_ack        : {hidden: true}
        policies_id       : {hidden: true}
    ...
    conditional {
      if ($input.doc_update) {
        ...
        db.add my_policies {
          ...
          data = {
            created_at        : "now"
            companies_id      : $my_policies1.companies_id
            policies_id       : $my_policies1.policies_id
            global_ack        : $my_policies1.global_ack
            validation_comment: $my_policies1.validation_comment
            date              : $input.date
            renewal_date      : $input.renewal_date
            user              : $my_policies1.user
            archived          : false
            document          : $var_1
          }
        ...
        db.edit my_policies {
          ...
          field_value = $input.id
          enforce_hidden_fields = false
          data = {date: $input.date, renewal_date: $input.renewal_date}
        } as $my_policies2
      }
    }
